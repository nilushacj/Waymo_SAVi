...
#   # Compute the difference for each frame.
#   diff = max_depth - min_depth  # shape: (6, 1, 1, 1)
...
#   converted_depth = min_depth + normalized_depth * diff  # shape: (6, H, W, 1)
...
class FromWaymoOpen:
  ...
  def __call__(self, features: Features) -> Features:
    ...
    # ---- Get seed ----
    if "rng" in features:
      features_new[SEED_KEY] = features.pop("rng") #OK
    ...
    features_new[self.video_key] = video #OK

    # ---- Store video shape (e.g. for correct evaluation metrics). ----
    features_new[self.shape_key] = tf.shape(video) #OK
    #print(f'cam image shape: {video.get_shape()}')
    # ---- Store padding mask ----
    features_new[self.padding_mask_key] = tf.cast(
      tf.ones_like(video)[..., 0], tf.uint8) #OK

    # ---- Extract depth ----
    depth_range     = features[self.camera_key]["depth_range"] #tf.constant([3.0, 80.0], dtype=tf.float32)
...
class VideoFromTfds:
  ...
  def __call__(self, features: Features) -> Features:
    ...
    features_new = {}
    #print(f" ****** VIDEO FROM TFDS features dict:{features.keys()} ******")
    ...
    #print(f" ****** VIDEO FROM TFDS NEW features dict:{features_new.keys()} ******")
    #print(f" ------------------------------------------------------------- ")
...
class SparseToDenseAnnotation:
  ...
  def __call__(self, features: Features) -> Features:
    #print(f" ****** SparseToDenseAnnotation features dict:{features.keys()} ******")
    #print(f" ------------------------------------------------------------- ")
    ...
    #print(f" ****** SparseToDenseAnnotation NEW features dict:{features_new.keys()} ******")
    #print(f" ------------------------------------------------------------- ")
    return features_new
...
class VideoPreprocessOp(abc.ABC):
  ...
  def __call__(self, features: Features) -> Features:
    #print(f" ****** VideoPreprocessOp features dict:{features.keys()} ******")
    #print(f" ------------------------------------------------------------- ")
    ...
    #print(f" ****** VideoPreprocessOp NEW features dict:{features.keys()} ******")
    #print(f" ------------------------------------------------------------- ")
    return features
  ...
  def apply(self, tensor: tf.Tensor, key: str,
    ...
    Args:
      ...
      video_shape: The shape of the video (which is necessary for some
        transformations).
    """
    #print(f" ****** VideoPreprocessOp APPLY CALLED ******")
...
class RandomVideoPreprocessOp(VideoPreprocessOp):
  ...
  def __call__(self, features: Features) -> Features:
    #print(f" ****** RandomVideoPreprocessOp features dict:{features.keys()} ******")
    #print(f" ------------------------------------------------------------- ")
    ...
    #print(f" ****** RandomVideoPreprocessOp NEW features dict:{features.keys()} ******")
    #print(f" ------------------------------------------------------------- ")
    return features
  ...
  def apply(self, tensor: tf.Tensor, seed: tf.Tensor, key: str,
    ...
    Args:
      ...
      video_shape: The shape of the video (which is necessary for some
        transformations).
    """
    #print(f" ****** RandomVideoPreprocessOp APPLY CALLED ******")
    pass
...
class TemporalRandomStridedWindow(RandomVideoPreprocessOp):
  ...
  def _apply(self, tensor: tf.Tensor, seed: Sequence[int],
    ...
    #print(f" ****** TemporalRandomStridedWindow _APPLY CALLED ******")
    #print(f" ------------------------------------------------------------- ")
  ...
  def apply(self, tensor, seed, key=None, video_shape=None):
    #print(f" ****** TemporalRandomStridedWindow APPLY CALLED ******")
    #print(f" ------------------------------------------------------------- ")
    """See base class."""
    del video_shape
    if key == self.boxes_key:
...
class TransformDepth:
  ...
  def __call__(self, features: Features) -> Features:
    #print(f" ****** TransformDepth features dict:{features.keys()} ******")
    #print(f" ------------------------------------------------------------- ")
    ...
    #print(f" ****** TransformDepth NEW features dict:{features.keys()} ******")
    #print(f" ------------------------------------------------------------- ")
    return features
...
class RandomResizedCrop(RandomVideoPreprocessOp):
  ...
  def apply(self, tensor: tf.Tensor, seed: tf.Tensor, key: str,
    ...
    #print(f" ****** RandomResizedCrop APPLY CALLED ******")
    #print(f" ------------------------------------------------------------- ")
  ...
  def sample_augmentation_params(self, video_shape: tf.Tensor, rng: tf.Tensor):
    ...
    #print(f" ****** RandomResizedCrop sample_augmentation_params CALLED ******")
    #print(f" ------------------------------------------------------------- ")
  ...
  def estimate_transformation(self, param: tf.Tensor, video_shape: tf.Tensor
    ...
    """
    #print(f" ****** RandomResizedCrop estimate_transformation CALLED ******")
    #print(f" ------------------------------------------------------------- ")
    del video_shape
